...
module.exports = app => {
  ...
  return function mockHttpclient(mockUrl, mockMethod, mockResult) {
    ...
    if (app.config.httpclient.useHttpClientNext) {
      ...
      let mockConfigs = app.__httpclientNextMockConfigs;
      if (!mockConfigs) {
        mockConfigs = [];
        mm(app, '__httpclientNextMockConfigs', mockConfigs);
      }
      ...
      if (typeof mockUrl === 'string') {
        ...
        pathname = path => {
          console.warn(path, orginalPathname);
      ...
      mockMethod.forEach(function(method) {
        ...
        const mockScope = mockPool.intercept({
          ...
          if (mockConfigIndex >= 0) {
            mockConfigIndex = app.__httpclientNextMockConfigIndex;
            mockResult = mockConfigs[mockConfigIndex].mockResult;
