...
class ClusterApplication extends Coffee {
  ...
  _callFunctionOnAppWorker(method, args) {
    for (let i = 0; i < args.length; i++) {
      ...
      if (typeof arg === 'function') {
        ...
        for (const key in arg) {
          // The properties on Error instance are not enumerable.
...
module.exports = options => {
  ...
  if (options.cache && clusters.has(options.baseDir)) {
    ...
    if (clusterApp.closed !== true) return clusterApp;
    ...
    get(target, prop) {
      ...
      if (typeof method === 'string' && /^mock\w+$/.test(method) && target[method] === undefined) {
        return function mockProxy(...args) {
          target._callFunctionOnAppWorker(method, args);
        };
      }
