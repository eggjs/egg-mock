...
class MockApplication extends EventEmitter {
  constructor(options) {
    ...
    .catch(err => {
      // if Application isn't initialized, can't get `ready` from app.
      if (!this[APP_INIT]) {
        this.emit('error', err);
      }
      this.ready(err);
...
module.exports = function(options) {
  ...
  if (options.cache && apps.has(options.baseDir)) {
    ...
    // the cache will be deleted by app.close(),
    // so it seems that the else logical will never be run.
    if (!app.closed) {
      return app;
    }
  ...
  app = new Proxy(app, {
    ...
    set(target, prop, value) {
      if (MOCK_APP_METHOD.includes(prop)) {
        if (prop === 'closed') target[prop] = value;
        return true;
      }
