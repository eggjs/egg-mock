...
function bindMessenger(Application, agent) {
  ...
  return class MessengerApplication extends Application {
    constructor(options) {
      ...
      // agent will wrap and restore `sendTo`, so only could proxy it after ready
      // https://github.com/eggjs/egg/blob/master/lib/agent.js#L72
      agentMessenger.on('egg-ready', () => {
        agentMessenger.opids = [ process.pid ];
        agentMessenger.sendTo = new Proxy(agentMessenger.sendTo, {
          // eslint-disable-next-line no-unused-vars
          apply(target, thisArg, [ pid, action, data ]) {
            return self._sendMessage.call(self, target, thisArg, [ action, data, 'app' ]);
          },
        });
      });
    }
