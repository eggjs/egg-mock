...
function getAgent(options) {
  ...
  const egg = require(options.customEgg);
  const MockAgent = extendsEggAgent(egg, options);
  return new MockAgent(options);
}

function extendsEggAgent(egg) {
  const symbol = egg.symbol;
  // detect port before load
  class MockAgent extends egg.Agent {
    get [symbol.Loader]() {
      ...
      class MockLoader extends CustomLoader {
        load() {
          ...
          detectPort((err, port) => {
            ...
            self._options.clusterPort = port;
            super.load();
            done();
          });
        }
      ...
      class MockLoader extends CustomLoader {
        load() {
          ...
          options.agent.ready().then(() => {
            self._options.clusterPort = self.agent._options.clusterPort;
            super.load();
            done();
          }).catch(done);
        }
