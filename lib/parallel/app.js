...
class MockApplication extends Base {
  ...
  async _init() {
    ...
    this.options.clusterPort = this.options.clusterPort || process.env.EGG_MOCK_CLUSTER_PORT;
    if (!this.options.clusterPort) {
      throw new Error('cannot get env.EGG_MOCK_CLUSTER_PORT, parallel run fail');
    }
